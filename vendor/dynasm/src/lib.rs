...
macro_rules! emit_error {
    ($span:expr, $message:literal $(, $arg:tt)* $(,)?) => {{
        $crate::diagnostics::emit_error(&$span, format!($message $(, $arg)*));
    }};
    ($span:expr, $message:expr $(,)?) => {{
        $crate::diagnostics::emit_error(&$span, $message.to_string());
    }};
}
...
mod diagnostics {
    ...
    thread_local! {
        static ERRORS: RefCell<Vec<TokenStream>> = RefCell::new(Vec::new());
    }

    pub(crate) trait DiagnosticSpan {
        fn diagnostic_span(&self) -> Span;
    }

    impl<T> DiagnosticSpan for &T
    where
        T: Spanned + ?Sized,
    {
        fn diagnostic_span(&self) -> Span {
    ...
    pub(crate) fn emit_error<S>(site: S, message: String)
    where
        S: DiagnosticSpan,
    {
        let span = site.diagnostic_span();
        ERRORS.with(|errors| {
            errors.borrow_mut().push(quote_spanned! { span=>
                compile_error!(#message);
            });
        });
    }

    pub(crate) fn append_errors(mut tokens: TokenStream) -> TokenStream {
        ERRORS.with(|errors| {
...
pub fn dynasm(tokens: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    diagnostics::append_errors(serialize::serialize(&dynasm.target, dynasm.stmts)).into()
}
...
pub fn dynasm_backwards(tokens: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    // serialize the resulting output into tokens
    diagnostics::append_errors(serialize::serialize(&dynasm.target, stmts)).into()
}
